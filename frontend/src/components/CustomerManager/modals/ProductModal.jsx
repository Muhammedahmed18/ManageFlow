...
import { X, Image, ChevronLeft, ChevronRight, ChevronDown, Check, Loader2, Lock } from 'lucide-react';
...
const ProductModal = ({
  ...
  // Initialize form and handle categories
  useEffect(() => {
    ...
    if (selectedProduct) {
      ...
      if (tpl && tpl.status !== 'draft') {
        ...
        tpl.fields.forEach(field => {
          ...
          if (!usedFieldIds.has(fid)) {
            // Set default value for default_value fields, empty string for others
            initialFieldValues[fid] = field.type === 'default_value' ? field.default_value || '' : '';
          }
      ...
      } else if (tpl && tpl.status === 'draft') {
        // Template exists but is now a draft - clear template selection
        setSelectedTemplate(null);
        setTemplateFields([]);
        setErrors(prev => ({ ...prev, template: 'The selected template is now in draft status and cannot be used. Please select a different template.' }));
      }
  
      setFormData({
        name: selectedProduct.name || '',
        custom_id: selectedProduct.custom_id || '',
        template_id: tpl && tpl.status !== 'draft' ? selectedProduct.template?.id : '',
  ...
  const handleTemplateChange = (templateId) => {
    ...
    if (tpl) {
      ...
      // Initialize values for all fields in the template
      const newFieldValues = {};
      tpl.fields.forEach(field => {
        // Use existing value if available, otherwise use default value for default_value fields
        const existingValue = formData.field_values[field.id];
        if (existingValue !== undefined) {
          newFieldValues[field.id] = existingValue;
        } else if (field.type === 'default_value') {
                ...
                <div className="col-span-1 md:col-span-2 space-y-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {templates.filter(template => template.status !== 'draft').length === 0 ? (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <span className={`${selectedCategoryPath.length ? 'text-gray-900' : 'text-gray-400'}`}>
                          {selectedCategoryPath.length ? selectedCategoryPath.map(c => c.name).join(' › ') : 'Select category (optional)'}
                        </span>
                      ...
                      {activeAccordion === 'basic' && (
                        ...
                        >
                          <div className="p-3 pt-0 grid grid-cols-1 md:grid-cols-2 gap-4">
                            {templateFields.filter(f => ['text', 'textarea', 'number', 'currency', 'default_value'].includes(f.type)).map((field) => (
